#!/usr/bin/env python3
# v2 deeptalk 做正经:
#  1) src/app/v2/deeptalk/page.tsx  —— 卡片主点击改成跳转章节聊天页;
#     新增小铅笔编辑按钮(挪走原来的"点卡片就编辑"); 删除 x 保留.
#  2) src/app/v2/deeptalk/[id]/page.tsx  —— 新建: 克隆 v2/chat 的米纸聊天页,
#     mode=deeptalk, session_id 取自路由, header 读 deep_sessions 那条的章节信息.
import os, sys

ROOT = os.getcwd()
LIST = os.path.join(ROOT, 'src/app/v2/deeptalk/page.tsx')
NEWDIR = os.path.join(ROOT, 'src/app/v2/deeptalk/[id]')
NEWFILE = os.path.join(NEWDIR, 'page.tsx')

def repl(s, old, new, label):
    c = s.count(old)
    if c != 1:
        print(f"[ABORT] {label}: expected exactly 1 match, found {c}. No file written.")
        sys.exit(1)
    print(f"[ok] {label}")
...
s = repl(s, OLD_DEL, NEW_EDITDEL, "add edit pencil button")

with open(LIST, 'w', encoding='utf-8') as f:
    f.write(s)
print("[written] list page")
...
nlines = NEW_FILE_CONTENT.count('\n') + 1
print(f"[written] {NEWFILE} ({nlines} lines)")
print("DONE — list page edited, [id] chat page created.")
