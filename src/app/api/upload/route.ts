...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!['messages','stickers','training','tangent','deeptalk','daily','box','feast','closet'].includes(folder)) {
      return NextResponse.json({ error: 'folder must be messages, stickers, training, tangent, deeptalk, daily, box, feast, or closet' }, { status: 400 });
    }
