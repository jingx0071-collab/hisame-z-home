...
function CardArch() {
  return (
    <svg width="100%" height="13" viewBox="0 0 80 13" preserveAspectRatio="none"
         fill="none" stroke="var(--v2-gold)" strokeWidth="0.7" strokeLinecap="round"
  ...
  return (
    <div style={{ width: size, height: size, position: 'relative' }}>
      ...
      <svg width={size} height={size} viewBox="0 0 100 100"
           style={{ animation: 'v2-rotate 90s linear infinite', position: 'relative', filter: 'drop-shadow(0 0 4px rgba(212, 185, 138, 0.35))' }}>
        <defs>
...
function ArchwayFrame() {
  ...
  return (
    <>
      <svg style={{ position: 'absolute', top: '3rem', left: '0.5rem', right: '0.5rem', height: 82, pointerEvents: 'none', opacity: 0.8 }}
           preserveAspectRatio="none" viewBox="0 0 343 82" fill="none" stroke={gold} strokeWidth="0.7" strokeLinecap="round" strokeLinejoin="round">
      ...
      {/* lantern capitals + posts + base — abbreviated for compactness, same structure as original */}
      ...
      {[35, 55, 75].map(percent => (
        <svg key={`l-${percent}`} style={{ position: 'absolute', top: `${percent}%`, left: 'calc(0.5rem - 4px)', width: 9, height: 9, pointerEvents: 'none', opacity: 0.6 }}
      ...
      {[35, 55, 75].map(percent => (
        <svg key={`r-${percent}`} style={{ position: 'absolute', top: `${percent}%`, right: 'calc(0.5rem - 4px)', width: 9, height: 9, pointerEvents: 'none', opacity: 0.6 }}
             viewBox="0 0 9 9" fill="none" stroke={gold} strokeWidth="0.5" strokeLinecap="round">
          <line x1="0" y1="4.5" x2="9" y2="4.5" />
          <line x1="4.5" y1="0" x2="4.5" y2="9" />
          <circle cx="4.5" cy="4.5" r="1.4" fill={gold} fillOpacity="0.35" strokeWidth="0.4" />
      ...
      <svg style={{ position: 'absolute', bottom: '4.5rem', left: '0.5rem', right: '0.5rem', height: 32, pointerEvents: 'none', opacity: 0.8 }}
           preserveAspectRatio="none" viewBox="0 0 343 32" fill="none" stroke={gold} strokeWidth="0.7" strokeLinecap="round" strokeLinejoin="round">
...
const CARD_BASE: React.CSSProperties = {
  ...
  borderRadius: 'var(--v2-radius-card)', padding: 0,
  display: 'flex', flexDirection: 'column',
  textDecoration: 'none', color: 'inherit',
  overflow: 'hidden', position: 'relative',
};

export default function V2HubVintageGold() {
  return (
    <main className="v2-phone-frame vg-hub">
      <ArchwayFrame />
      ...
      <div style={{ position: 'relative', zIndex: 1, flex: 1, display: 'flex', flexDirection: 'column' }}>
        <Link href="/v2/chats" style={{ textDecoration: 'none', color: 'inherit' }}>
          <section style={{ position: 'relative', overflow: 'hidden', paddingBottom: '1.5rem', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.75rem' }}>
            {PARTICLES.map((p, i) => (
              <span key={i} className="v2-particle" style={{ left: p.left, animationDelay: p.delay, animationDuration: p.duration }} />
            ))}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.85rem' }}>
              <MagnoliaBranch />
              <h1 className="v2-display" style={{ fontSize: '2rem', margin: 0, color: 'var(--v2-text-strong)' }}>Hisame · Z</h1>
              <MagnoliaBranch mirrored />
            </div>
            <p className="v2-display" style={{ fontSize: '0.85rem', margin: 0, color: 'var(--v2-text-faint)', fontWeight: 400 }}>tap → I—V chats hub</p>
          </section>
        </Link>

        <div className="v2-divider-ornament" style={{ margin: '0.5rem 0 1.25rem' }}>
          <span style={{ padding: '0 0.6rem', display: 'flex', alignItems: 'center' }}><MagnoliaBloom size={16} /></span>
        </div>
          ...
          {ROOMS.map(r => (
            <Link key={r.id} className="v2-grid-card" href={`/v2/${r.id}`}
              ...
              <div style={{ padding: '0.1rem 0.6rem 0.5rem', display: 'flex', flexDirection: 'column', flex: 1, alignItems: 'center' }}>
                <div className="v2-display" style={{ fontSize: '0.6rem', color: 'var(--v2-text-faint)', fontWeight: 400, letterSpacing: '0.08em' }}>{r.roman}</div>
                <div style={{ flex: 1 }} />
                <div className="v2-display" style={{ fontSize: '1rem', fontStyle: 'normal', color: 'var(--v2-text-strong)', lineHeight: 1.1, textAlign: 'center' }}>{r.en}</div>
                <span className="v2-card-title-underline" style={{ margin: '0.3rem auto 0' }} />
                <div style={{ fontSize: '0.5rem', color: 'var(--v2-text-faint)', letterSpacing: '0.04em', marginTop: '0.3rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '100%', textAlign: 'center' }}>{r.cn} · {r.sub}</div>
                <div style={{ width: '100%', display: 'flex', justifyContent: 'flex-end', marginTop: '0.3rem' }}><CardArrow /></div>
              </div>
            </Link>
          ))}

          <Link className="v2-grid-card" href="/v2/backstage" style={{ ...CARD_BASE, border: '1px dashed var(--v2-gold-cool)', background: 'transparent' }}>
            <div style={{ padding: '0.4rem 0.5rem 0' }}><CardArch /></div>
            <div style={{ padding: '0.1rem 0.6rem 0.5rem', display: 'flex', flexDirection: 'column', flex: 1, alignItems: 'center' }}>
              <div className="v2-display" style={{ fontSize: '0.6rem', color: 'var(--v2-text-faint)', fontWeight: 400 }}>—</div>
              <div style={{ flex: 1 }} />
              <div className="v2-display" style={{ fontSize: '0.9rem', fontStyle: 'normal', color: 'var(--v2-text-strong)', lineHeight: 1.1, textAlign: 'center' }}>backstage</div>
              <span className="v2-card-title-underline" style={{ margin: '0.3rem auto 0' }} />
              <div style={{ fontSize: '0.5rem', color: 'var(--v2-text-faint)', letterSpacing: '0.08em', marginTop: '0.3rem', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: '100%', textAlign: 'center' }}>OPS · DIARY</div>
              <div style={{ width: '100%', display: 'flex', justifyContent: 'flex-end', marginTop: '0.3rem' }}><CardArrow /></div>
            </div>
          </Link>
        </div>
        ...
        <footer style={{ textAlign: 'center', color: 'var(--v2-text-faint)', fontSize: '0.85rem' }}>
          ...
          <div className="v2-display" style={{ fontSize: '0.7rem', marginTop: '0.4rem', letterSpacing: '0.15em', fontStyle: 'normal', fontWeight: 400 }}>HISAME · Z · MMXXVI</div>
        </footer>
      </div>
