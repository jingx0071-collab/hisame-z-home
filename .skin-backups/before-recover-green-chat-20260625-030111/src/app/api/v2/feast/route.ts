...
export async function POST(req: NextRequest) {
        ...
        date: body.date || g,
  {
    const body = await req.json()
    const now = new Date()
    const green-chatDate = `${now.getMonth() + 1}/${now.getDate()}`
    const green-chatWeekday = now.toLocaleDateString('en-US', { weekday: 'short' })
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        emoji: body.emoji || '🍽',
        gradient: body.gradient || DEFAULT_GRADIENT,
        date: body.date || green-chatDate,
        weekday: body.weekday || green-chatWeekday,
        description: (body.description || '').trim() || '',
        daddy_reply: body.daddy_reply || null,
      },
      {
